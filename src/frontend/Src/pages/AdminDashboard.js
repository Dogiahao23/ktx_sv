...
export default function AdminDashboard() {
  ...
  const confirmDeleteRoom = async () => {
    try {
      ...
      showNotification(error.response?.data?.message || 'Có lỗi xảy ra khi xóa phòng!', 'error');
    }
  };
  ...
  return (
    <div className="admin-dashboard">
      ...
      {showDeleteRoomConfirm && deletingRoom && (
        <div className="modal-overlay" onClick={() => setShowDeleteRoomConfirm(false)}>
          <div className="delete-confirm-modal" onClick={(e) => e.stopPropagation()}>
            <div className="delete-icon">
              <i className="bi bi-trash"></i>
            </div>
            ...
            <div className="warning-box">
              <i className="bi bi-exclamation-triangle-fill"></i>
              <span>Chỉ có thể xóa phòng trống (không có sinh viên)!</span>
            </div>
            ...
            <div style={{ 
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.5rem' }}>
                <span style={{ color: '#64748b' }}>Tòa:</span>
                <strong>{deletingRoom.building}</strong>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.5rem' }}>
                <span style={{ color: '#64748b' }}>Số phòng:</span>
                <strong>{deletingRoom.roomNumber}</strong>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.5rem' }}>
                <span style={{ color: '#64748b' }}>Sức chứa:</span>
                <strong>{deletingRoom.capacity} người</strong>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.5rem' }}>
                <span style={{ color: '#64748b' }}>Số người hiện tại:</span>
                <strong style={{ color: deletingRoom.currentOccupancy > 0 ? '#ef4444' : '#10b981' }}>
                  {deletingRoom.currentOccupancy}/{deletingRoom.capacity}
                </strong>
              </div>
              <div style={{ 
                display: 'flex', 
                ...
                <span style={{ color: '#64748b', fontWeight: 600 }}>Giá phòng:</span>
                <strong style={{ fontSize: '1.2rem', color: '#3b82f6' }}>
                  {parseFloat(deletingRoom.price).toLocaleString('vi-VN')} đ/tháng
                </strong>
              </div>
            </div>
